...
purify-ts provides Haskell-inspired algebraic data types that eliminate `null`, `undefined`,
and thrown exceptions from your code.
...
const findUser = (id: number): Maybe<string> =>
  id > 0 ? Just(`user-${id}`) : Nothing
...
const buildConfig = (
  host: Maybe<string>,
  port: Maybe<number>
): Maybe<{ host: string; port: number }> =>
  Maybe.sequence([host, port] as const)
    .map(([h, p]) => ({ host: h, port: p }))
```
...
const parseUser = (raw: unknown): Either<string, User> =>
...
const safeHead = <A>(xs: A[]): Maybe<A> =>
  NonEmptyList.fromArray(xs).map(nel => nel[0])
...
const getFormattedPrice = (id: number): EitherAsync<string, string> =>
  fetchProduct(id).map(formatPrice)
```

...
export const isFailure = <E, A>(va: Validation<E, A>): va is Failure<E> =>
  va.tag === 'Failure'

/** Test if a Validation is Success. */
export const isSuccess = <E, A>(va: Validation<E, A>): va is Success<A> =>
  va.tag === 'Success'

/** Lift an Either into Validation. */
export const fromEither = <E, A>(ea: Either<E, A>): Validation<E, A> =>
  ea.tag === 'Left' ? Failure([ea.left]) : Success(ea.right)
...
export const toEither = <E, A>(
  ...
  va.tag === 'Failure'
    ? { tag: 'Left' as const, left: va.errors }
    : { tag: 'Right' as const, right: va.value }
...
export const sequence = <E, A>(
  validations: readonly Validation<E, A>[]

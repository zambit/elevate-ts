 ...
 */
const unsafeBrand = <A>(arr: readonly A[]): NonEmptyList<A> => arr as NonEmptyList<A>;
 ...
 */
export const fromArray = <A>(arr: readonly A[]): Maybe.Maybe<NonEmptyList<A>> => (arr.length > 0 ? Maybe.Just(unsafeBrand(arr)) : Maybe.Nothing);
 ...
 * @returns The array cast as NonEmptyList.
 */
export const fromArrayUnsafe = <A>(arr: readonly A[]): NonEmptyList<A> => unsafeBrand(arr);
 ...
 */
export const last = <A>(nel: NonEmptyList<A>): A => nel[nel.length - 1];
 ...
 */
export const init = <A>(nel: NonEmptyList<A>): readonly A[] => nel.slice(0, -1);

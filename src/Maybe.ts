...
export const toNullable = <A>(ma: Maybe<A>): A | null =>
  ma.tag === 'Just' ? ma.value : null

/** Lift to 0 or 1 element array. */
export const toArray = <A>(ma: Maybe<A>): readonly A[] =>
  ma.tag === 'Just' ? [ma.value] : []
...
export const catMaybes = <A>(maybes: readonly Maybe<A>[]): readonly A[] =>
  maybes.flatMap((ma) => (ma.tag === 'Just' ? [ma.value] : []))
...
justProto['fantasy-land/ap'] = function <A, B>(
  this: Just<(a: A) => B>,
  ma: Maybe<A>
) {
  return ap(this)(ma)
}
justProto['fantasy-land/chain'] = function <A, B>(
  this: Just<A>,
  f: (a: A) => Maybe<B>
) {
  return f(this.value)
...
justProto['fantasy-land/reduce'] = function <A, B>(
  this: Just<A>,
  f: (b: B, a: A) => B,
  b: B
) {
...
nothingProto['fantasy-land/reduce'] = function <A, B>(
  this: Nothing,
  f: (b: B, a: A) => B,
  b: B
) {

 ...
 */
export const last = <A>(arr: readonly A[]): A | undefined => (arr.length > 0 ? arr[arr.length - 1] : undefined);
 ...
 */
export const init = <A>(arr: readonly A[]): readonly A[] => (arr.length > 0 ? arr.slice(0, -1) : []);
 ...
 */
export const uncons = <A>(arr: readonly A[]): Maybe.Maybe<readonly [A, readonly A[]]> => (arr.length > 0 ? Maybe.Just([arr[0], arr.slice(1)]) : Maybe.Nothing);
 ...
 */
export const cons =
  <A>(a: A): ((arr: readonly A[]) => readonly A[]) =>
  (arr) => [a, ...arr];
 ...
 */
export const take =
  <A>(n: number): ((arr: readonly A[]) => readonly A[]) =>
  (arr) =>
    arr.slice(0, n);
...
export const takeWhile =
  <A>(predicate: (a: A) => boolean): ((arr: readonly A[]) => readonly A[]) =>
  (arr) => {
    let i = 0;
...
export const partition =
  <A>(predicate: (a: A) => boolean): ((arr: readonly A[]) => readonly [readonly A[], readonly A[]]) =>
  (arr) => {
    const left: A[] = [];
    const right: A[] = [];
    for (const a of arr) {
...
export const groupBy =
  <A>(eq: (a: A, b: A) => boolean): ((arr: readonly A[]) => readonly (readonly A[])[]) =>
  (arr) => {
 ...
 */
export const nubBy =
  <A>(eq: (a: A, b: A) => boolean): ((arr: readonly A[]) => readonly A[]) =>
  (arr) => {
    if (arr.length === 0) return [];
 ...
 */
export const sortBy =
  <A>(ord: (a: A, b: A) => number): ((arr: readonly A[]) => readonly A[]) =>
  (arr) =>
    [...arr].sort(ord);
...
export const zipWith =
  <A, B, C>(f: (a: A, b: B) => C): ((arr2: readonly B[]) => (arr1: readonly A[]) => readonly C[]) =>
  (arr2) =>
  (arr1) => {
    const len = Math.min(arr1.length, arr2.length);
    const result: C[] = [];
 ...
 */
export const unzip = <A, B>(pairs: readonly (readonly [A, B])[]): readonly [readonly A[], readonly B[]] => {
  const as: A[] = [];
  const bs: B[] = [];
...
export const intersperse =
  <A>(sep: A): ((arr: readonly A[]) => readonly A[]) =>
  (arr) => {
    if (arr.length === 0) return [];
    const result: A[] = [arr[0]];
    for (let i = 1; i < arr.length; i++) {
 ...
 */
export const transpose = <A>(matrix: readonly (readonly A[])[]): readonly (readonly A[])[] => {

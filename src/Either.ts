...
export const isLeft = <L, R>(ea: Either<L, R>): ea is Left<L> =>
  ea.tag === 'Left'

/** Test if Either is Right. */
export const isRight = <L, R>(ea: Either<L, R>): ea is Right<R> =>
  ea.tag === 'Right'
...
export const toNullable = <L, R>(ea: Either<L, R>): R | null =>
  ea.tag === 'Right' ? ea.right : null
...
/** Swap Left and Right. */
export const swap = <L, R>(ea: Either<L, R>): Either<R, L> =>
  ea.tag === 'Left' ? Right(ea.left) : Left(ea.right)

/** Wrap throwing function in Either. */
export const tryCatch =
  <L, R>(f: () => R, onError: (e: unknown) => L): Either<L, R> => {
    try {
      return Right(f())
    } catch (e) {
...
export const partitionEithers = <L, R>(
...
/** Extract all Lefts. */
export const lefts = <L, R>(eithers: readonly Either<L, R>[]): readonly L[] =>
  eithers.flatMap((ea) => (ea.tag === 'Left' ? [ea.left] : []))

/** Extract all Rights. */
export const rights = <L, R>(eithers: readonly Either<L, R>[]): readonly R[] =>
  eithers.flatMap((ea) => (ea.tag === 'Right' ? [ea.right] : []))

/** Sequence all-or-Left. */
export const sequence = <L, A>(
  eithers: readonly Either<L, A>[]
): Either<L, readonly A[]> => {
  const result: A[] = []
  for (const ea of eithers) {
...
rightProto['fantasy-land/ap'] = function <L, A, B>(
  this: Right<(a: A) => B>,
  ea: Either<L, A>
) {
  return ap(this)(ea)
}
rightProto['fantasy-land/chain'] = function <L, A, B>(
  this: Right<A>,
  f: (a: A) => Either<L, B>
) {
  return f(this.right)
}
rightProto['fantasy-land/bimap'] = function <L, L2, A, B>(
  this: Right<A>,
  f: (l: L) => L2,
  g: (a: A) => B
) {
...
rightProto['fantasy-land/equals'] = function <L, A>(
  this: Right<A>,
  other: Either<L, A>
) {
...
leftProto['fantasy-land/reduce'] = function <L, A, B>(
  this: Left<L>,
  f: (b: B, a: A) => B,
  b: B
) {

 ...
 */
export const fromPromise = <A>(p: Promise<A>): MaybeAsync<A> => MaybeAsync(() => p.then((a) => Maybe.Just(a)).catch(() => Maybe.Nothing));
 ...
 */
export const map =
  <A, B>(f: (a: A) => B): ((ma: MaybeAsync<A>) => MaybeAsync<B>) =>
  (ma) =>
    MaybeAsync(() => ma.run().then((maybe) => Maybe.map(f)(maybe)));
 ...
 */
export const chain =
  <A, B>(f: (a: A) => MaybeAsync<B>): ((ma: MaybeAsync<A>) => MaybeAsync<B>) =>
  (ma) =>
    MaybeAsync(async () => {
 ...
 */
export const ap =
  <A, B>(mf: MaybeAsync<(a: A) => B>): ((ma: MaybeAsync<A>) => MaybeAsync<B>) =>
  (ma) =>
    MaybeAsync(async () => {
 ...
 */
export const alt =
  <A>(alt_ma: MaybeAsync<A>): ((ma: MaybeAsync<A>) => MaybeAsync<A>) =>
  (ma) =>
    MaybeAsync(async () => {
      const maybe = await ma.run();
 ...
 */
export const filter =
  <A>(predicate: (a: A) => boolean): ((ma: MaybeAsync<A>) => MaybeAsync<A>) =>
  (ma) =>
    MaybeAsync(() => ma.run().then((maybe) => Maybe.filter(predicate)(maybe)));
...
export const getOrElse =
  <A>(a: A): ((ma: MaybeAsync<A>) => Promise<A>) =>
  (ma) =>
...
export const getOrElseL =
  <A>(f: () => Promise<A>): ((ma: MaybeAsync<A>) => Promise<A>) =>
  (ma) =>
 ...
 */
export const fold =
  <A, B>(onNothing: B, onJust: (a: A) => Promise<B>): ((ma: MaybeAsync<A>) => Promise<B>) =>
  (ma) =>
    ma.run().then(async (maybe) => {
 ...
 */
export const catMaybes = <A>(maybes: readonly MaybeAsync<A>[]): Promise<readonly A[]> =>
  Promise.all(maybes.map((m) => m.run())).then((results) => results.flatMap((maybe) => (maybe.tag === 'Just' ? [maybe.value] : [])));

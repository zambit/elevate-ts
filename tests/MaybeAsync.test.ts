...
describe('MaybeAsync', () => {
  ...
  describe('chain', () => {
    it('chain flattens nested MaybeAsync', async () => {
      const maa = MaybeAsync.liftMaybe(Maybe.Just(5));
      const chained = MaybeAsync.chain((x) => MaybeAsync.liftMaybe(Maybe.Just(x * 2)))(maa);
      const result = await chained.run();
      expect(result).toEqual(Maybe.Just(10));
    });
  ...
  describe('getOrElseL', () => {
    ...
    it('getOrElseL computes default for Nothing', async () => {
      const maa = MaybeAsync.liftMaybe(Maybe.Nothing);
      const result = await MaybeAsync.getOrElseL(() => Promise.resolve(99))(maa);
      expect(result).toBe(99);
    });
  });
  ...
  describe('fold', () => {
    it('fold calls onJust for Just', async () => {
      const maa = MaybeAsync.liftMaybe(Maybe.Just(5));
      const result = await MaybeAsync.fold('nothing', (x) => Promise.resolve(`just: ${x}`))(maa);
      expect(result).toBe('just: 5');
    });

    it('fold calls onNothing for Nothing', async () => {
      const maa = MaybeAsync.liftMaybe(Maybe.Nothing);
      const result = await MaybeAsync.fold('nothing', (x: number) => Promise.resolve(`just: ${x}`))(maa);
  ...
  describe('catMaybes', () => {
    it('catMaybes collects Just values', async () => {
      const maybes = [MaybeAsync.liftMaybe(Maybe.Just(1)), MaybeAsync.liftMaybe(Maybe.Nothing), MaybeAsync.liftMaybe(Maybe.Just(2))];
      const result = await MaybeAsync.catMaybes(maybes);
      expect(result).toEqual([1, 2]);
    });

    it('catMaybes filters out Nothing', async () => {
      const maybes = [MaybeAsync.liftMaybe(Maybe.Nothing), MaybeAsync.liftMaybe(Maybe.Nothing)];
      const result = await MaybeAsync.catMaybes(maybes);
  ...
  describe('all', () => {
    it('all succeeds if all are Just', async () => {
      const maybes = [MaybeAsync.liftMaybe(Maybe.Just(1)), MaybeAsync.liftMaybe(Maybe.Just(2))];
    ...
    it('all returns Nothing if any is Nothing', async () => {
      const maybes = [MaybeAsync.liftMaybe(Maybe.Just(1)), MaybeAsync.liftMaybe(Maybe.Nothing)];
      const result = await MaybeAsync.all(maybes).run();
      expect(result).toEqual(Maybe.Nothing);
    });
  });

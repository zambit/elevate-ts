...
describe('Validation', () => {
  ...
  describe('traverse', () => {
    ...
    it('traverse collects all errors', () => {
      const f = (x: number) =>
        x > 0 ? Success(x * 2) : Failure([`${x} is not positive`])
  ...
  describe('error accumulation in complex chains', () => {
    it('combining validations with errors accumulates correctly', () => {
      ...
      const validateName = (f: Field) =>
        f.name.length > 0 ? Success(f) : Failure(['Name required'])
      const validateValue = (f: Field) =>
        f.value.length > 0 ? Success(f) : Failure(['Value required'])

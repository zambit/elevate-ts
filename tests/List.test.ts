...
describe('List', () => {
  ...
  describe('partition', () => {
    ...
    it('partition preserves order within groups', () => {
      const [true_vals, false_vals] = List.partition(
        (x: number) => x > 2
      )([1, 3, 2, 4, 2, 5])
  ...
  describe('zip and zipWith', () => {
    it('zip pairs arrays and returns remainders', () => {
      ...
      expect(pairs).toEqual([[1, 10], [2, 20], [3, 30]])
    ...
    it('zipWith with string function', () => {
      ...
      expect(List.zipWith(concat)(['x', 'y'])(['a', 'b', 'c'])).toEqual([
        'ax',
        'by',
      ])
    })
  ...
  describe('transpose', () => {
    it('transpose transposes a matrix', () => {
      ...
      expect(List.transpose(matrix)).toEqual([[1, 4], [2, 5], [3, 6]])
    })
  ...
  describe('composition examples', () => {
    ...
    it('partition then transform', () => {
      const [evens, odds] = List.partition((x: number) => x % 2 === 0)([
        1, 2, 3, 4, 5, 6,
      ])

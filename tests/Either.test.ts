...
describe('Either', () => {
  ...
  describe('predicate lifting', () => {
    it('fromPredicate creates Right if predicate holds', () => {
      ...
      expect(fromPredicate(pred, (x) => `${x} is not positive`)(5)).toEqual(Right(5));
    });

    it('fromPredicate creates Left if predicate fails', () => {
      const pred = (x: number) => x > 0;
      expect(fromPredicate(pred, (x) => `${x} is not positive`)(-5)).toEqual(Left('-5 is not positive'));
    });
  });
  ...
  describe('partitionEithers', () => {
    it('partitionEithers separates Lefts and Rights', () => {
      const eithers: Either<string, number>[] = [Right(1), Left('a'), Right(2), Left('b'), Right(3)];
  ...
  describe('lefts', () => {
    it('lefts extracts all Lefts', () => {
      const eithers: Either<string, number>[] = [Right(1), Left('a'), Right(2), Left('b')];
  ...
  describe('sequence', () => {
    ...
    it('sequence with any Left returns first Left', () => {
      const eithers: Either<string, number>[] = [Right(1), Left('error'), Right(3)];
    ...
    it('sequence short-circuits on first Left (fail-fast)', () => {
      const eithers: Either<string, number>[] = [Left('first'), Right(2), Right(3)];
  ...
  describe('traverse', () => {
    ...
    it('traverse with any Left', () => {
      const f = (x: number): Either<string, number> => (x > 2 ? Right(x * 2) : Left('too small'));
      const result = traverse(f)([1, 2, 3]);

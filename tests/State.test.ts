...
describe('State', () => {
  ...
  describe('complex scenarios', () => {
    ...
    it('stateful computation with multiple operations', () => {
      const computation = chain((initial) =>
        chain((doubled) =>
          chain(() => modify((s) => s + initial))(get())
        )(State(() => [initial * 2, initial]))
      )(get())
    ...
    it('gets used to extract values during computation', () => {
      const computation = chain((n) =>
        chain((x) => State(() => [x + n, n]))(gets((s) => s * 2))
      )(get())
    ...
    it('map and chain combined', () => {
      ...
      const s3 = chain((doubled) =>
        State(() => [doubled + 100, doubled])
      )(s2)

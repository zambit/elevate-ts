...
describe('EitherAsync', () => {
  ...
  describe('chain', () => {
    it('chain flattens nested EitherAsync', async () => {
      const eaa = EitherAsync.liftEither(Either.Right(5));
      const chained = EitherAsync.chain((x) => EitherAsync.liftEither(Either.Right(x * 2)))(eaa);
  ...
  describe('all', () => {
    it('all succeeds if all are Right', async () => {
      const eas = [EitherAsync.liftEither(Either.Right(1)), EitherAsync.liftEither(Either.Right(2))];
    ...
    it('all returns first Left if any is Left', async () => {
      const eas = [EitherAsync.liftEither(Either.Right(1)), EitherAsync.liftEither(Either.Left('error1')), EitherAsync.liftEither(Either.Left('error2'))];
      const result = await EitherAsync.all(eas).run();
  ...
  describe('lefts and rights', () => {
    it('lefts extracts all Left values', async () => {
      const eas = [EitherAsync.liftEither(Either.Right(1)), EitherAsync.liftEither(Either.Left('e1')), EitherAsync.liftEither(Either.Right(2)), EitherAsync.liftEither(Either.Left('e2'))];

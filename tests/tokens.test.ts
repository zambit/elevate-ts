...
describe('tokenConfig', () => {
  describe('colors', () => {
    it('contains exactly 10 neutral scale entries', () => {
      const neutralKeys = Object.keys(tokenConfig.colors).filter((k) => k.startsWith('neutral-'));
      expect(neutralKeys).toHaveLength(10);
      expect(neutralKeys).toEqual(['neutral-50', 'neutral-100', 'neutral-200', 'neutral-300', 'neutral-400', 'neutral-500', 'neutral-600', 'neutral-700', 'neutral-800', 'neutral-900']);
    });
    ...
    it('neutral scale lightness is strictly decreasing', () => {
      const neutralKeys = ['neutral-50', 'neutral-100', 'neutral-200', 'neutral-300', 'neutral-400', 'neutral-500', 'neutral-600', 'neutral-700', 'neutral-800', 'neutral-900'];
  ...
  describe('shape invariants', () => {
    it('top-level keys are correct', () => {
      ...
      expect(keys).toEqual(['colors', 'radius', 'shadows', 'spacing', 'transitions', 'typography', 'zIndex']);
    });

    it('typography contains correct sub-keys', () => {
      const keys = Object.keys(tokenConfig.typography).sort();
      expect(keys).toEqual(['fontFamilies', 'fontSizes', 'fontWeights', 'lineHeights']);
    });
  });
});

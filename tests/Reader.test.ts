...
import { Reader, ask, asks, local, map, ap, chain, runReader } from '../src/Reader.js';
...
describe('Reader', () => {
  ...
  describe('complex chaining scenarios', () => {
    it('chaining multiple Readers with environment access', () => {
      ...
      const r2 = chain((a) => asks((env: { a: number; b: number }) => a + env.b))(r1);
  ...
  describe('practical examples', () => {
    it('dependency injection pattern with Reader', () => {
      ...
      const query = chain((db) => map((timeout) => `Query ${db} with timeout ${timeout}`)(getTimeout))(getDbUrl);
    ...
    it('configuration-based computation', () => {
      ...
      const compute = chain((mult) => asks((e: typeof env) => mult + e.offset))(asks((e: typeof env) => e.multiplier));
